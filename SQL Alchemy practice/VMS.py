...
def create_booking(booking: ServiceBookingCreate, db: Session = next(get_db())):
...
def get_all_bookings(db: Session = next(get_db())):
    bookings = db.query(ServiceBooking).all()
    return bookings

@app.get("/bookings/{booking_id}", status_code=status.HTTP_200_OK)
def get_booking_by_id(booking_id: int, db: Session = next(get_db())):
...
def update_booking(booking_id: int, updated_booking: ServiceBookingCreate, db: Session = next(get_db())):
    booking = db.query(ServiceBooking).filter(ServiceBooking.id == booking_id).first()
    if not booking:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def delete_booking(booking_id: int, db: Session = next(get_db())):
    booking = db.query(ServiceBooking).filter(ServiceBooking.id == booking_id).first()
    if not booking:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
